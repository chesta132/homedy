...
type Share struct {
	...
	ReadOnly    SambaBool `ini:"read only,omitempty" json:"readOnly" example:"no" validate:"required,samba_bool"`
	Browsable   SambaBool `ini:"browsable,omitempty" json:"browsable" example:"yes" validate:"required,samba_bool"`
	ValidUsers  []string  `ini:"valid users,omitempty" json:"validUsers" example:"guest,chesta" validate:"required,min=1,dive,required"`
	AdminUsers  []string  `ini:"admin users,omitempty" json:"adminUsers" example:"root" validate:"required,min=1,dive,required"`
