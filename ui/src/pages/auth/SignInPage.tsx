...
export function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await api.auth.post<User>("/signin", {
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#0a0a0a] p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) => validate.validateField({ identifier: e.target.value })}
        ...
        <p className="mt-6 text-center text-sm text-[#555555]">
          ...
          <Link to="/signup" className="text-[#ededed] underline underline-offset-4 hover:text-white">
            Sign up
          </Link>
        </p>
      </motion.div>
